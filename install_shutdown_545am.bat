...
title PC Shutdown Schedule Installer (5:45 AM Only)
...
echo   - Shutdown at 5:45 AM (Every day)
echo.
...
:: ========================================
:: Create Shutdown PC Task (5:45 AM)
:: ========================================
echo Creating Shutdown PC task for 5:45 AM...
echo.
...
set "SHUTDOWN_SCRIPT=%SCRIPT_DIR%_shutdown_pc_545am.bat"
...
echo shutdown /s /f /t 60 /c "Scheduled shutdown at 5:45 AM"
) > "%SHUTDOWN_SCRIPT%"
...
set "SHUTDOWN_TASK_XML=%SCRIPT_DIR%_shutdown_task_545am.xml"
...
echo     ^<Description^>Shutdown PC for AgentFront at 5:45 AM^</Description^>
echo   ^</RegistrationInfo^>
echo   ^<Triggers^>
echo     ^<CalendarTrigger^>
echo       ^<StartBoundary^>2024-01-01T05:45:00^</StartBoundary^>
...
schtasks /create /tn "AgentFront_ShutdownPC_545AM" /xml "%SHUTDOWN_TASK_XML%" /f
if %errorLevel% equ 0 (
...
echo ========================================
echo.
echo Shutdown Task (5:45 AM):
schtasks /query /tn "AgentFront_ShutdownPC_545AM" /fo list 2>nul | findstr "TaskName Status Next"
...
echo.
echo Schedule:
echo   - PC shuts down at 5:45 AM (Every day)
echo.
echo NOTE: This fires at 5:45 on THIS PC's clock. It matches
echo       5:45 AM Japan time only if the PC clock is set to
echo       Japan time (JST, UTC+9). This PC currently reads UTC+7.
echo.
...
echo   schtasks /delete /tn "AgentFront_ShutdownPC_545AM" /f
echo.
echo ========================================
echo Press any key to exit...
pause >nul
